...
function setup(submission: unknown = { id: 's1', status: 'PENDING', vehicle: { chassis: 'LS6CME0P7TC914754' } }, receipt: unknown = null, extractor: ReceiptExtractor = new NoAiReceiptExtractor(), pendingByChassis: unknown = null, nearCandidates: unknown[] = []) {
  ...
  const prisma = {
    documentSubmission: { findUnique: vi.fn().mockResolvedValue(submission), findFirst: vi.fn().mockResolvedValue(pendingByChassis), findMany: vi.fn().mockResolvedValue(nearCandidates) },
    receiptImage: {
      create,
